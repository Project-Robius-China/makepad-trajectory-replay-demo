// P13.1: GeoMapView ported from makepad-map-widget (1.x DSL) to mobile_example (2.x splash).
//
// Strategy:
// - DrawMapTile / DrawMapMarker are kept as DrawQuad sub-classes via #[derive(Script, ScriptHook)] +
//   #[repr(C)]. Their shaders are registered in main.rs via `set_type_default() do
//   #(DrawMapTile::script_shader(vm)) { ... }` blocks (same pattern as DrawTrack/DrawWater/...).
// - GeoMapView is a Widget (#[derive(Script, ScriptHook, Widget)]) with #[uid] / #[walk] /
//   #[layout] / #[redraw] draw_tile / #[area] area, mirroring TrackCanvas.
// - The 1.x DrawColor / DrawText sub-widgets (scale bar, attribution, marker label) are dropped:
//   the host demo (mobile_example) renders its own labels via app-level Labels and does NOT
//   need the OSM attribution/scale bar overlay (this is a locked-position bg map). This keeps
//   the port focused on tile rendering + future pan/zoom; markers are kept (struct field is
//   #[rust] markers: Vec<MapMarker>) but drawn with a marker_color uniform only.
// - Action enum, hit testing, momentum, and tile cache logic are preserved verbatim.
// - 2.x widget_action signature is (uid, action) — no HeapLiveIdPath. Scope is unused for
//   action emission; we keep the &mut Scope arg for future use in handle_event but the action
//   call itself drops the path arg.

use makepad_widgets::*;

use super::tiles::{TileCache, TileCoord};
...
pub struct GeoMapView {
    ...
    #[redraw]
    #[live]
    pub draw_tile: DrawMapTile,

    // Markers
    #[live]
    draw_marker: DrawMapMarker,
    #[live(32.0)]
    pub marker_size: f64,
    #[rust]
    ...
    viewport_size: DVec2,
    #[rust]
    viewport_pos: DVec2, // Top-left position of viewport in absolute coords

    // Pinch zoom state
    #[rust]
    initial_pinch_distance: Option<f64>,
    #[rust]
    pinch_zoom_start: Option<f64>,

    // Momentum scrolling state
    #[rust]
    velocity_samples: Vec<(DVec2, f64)>, // (position, time in seconds)
    #[rust]
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, _scope: &mut Scope) {
        ...
        if let Event::TouchUpdate(te) = event {
            ...
            if te.touches.len() >= 2 {
                ...
                if let (Some(initial_distance), Some(start_zoom)) =
                    (self.initial_pinch_distance, self.pinch_zoom_start)
                {
                    ...
                    let new_zoom =
                        (start_zoom + zoom_delta).clamp(self.min_zoom, self.max_zoom);
        ...
        match event.hits(cx, self.area) {
            ...
            Hit::FingerUp(fe) if fe.is_primary_hit() => {
                ...
                if fe.is_over && is_tap {
                    ...
                    if let Some(marker_id) = self.find_marker_at_screen_pos(fe.abs) {
                        cx.widget_action(uid, GeoMapViewAction::MarkerTapped { id: marker_id });
                    } else {
                        let (lng, lat) = self.screen_to_geo(fe.abs);
                        cx.widget_action(uid, GeoMapViewAction::Tapped { lng, lat });
                    }
                ...
                self.velocity_samples.clear();
                if !is_tap {
                    self.emit_region_changed(cx, uid);
                }
            }
            Hit::FingerScroll(fe) => {
            ...
            Hit::FingerLongPress(fe) => {
                let (lng, lat) = self.screen_to_geo(fe.abs);
                cx.widget_action(uid, GeoMapViewAction::LongPressed { lng, lat });
            }
            _ => {}
        }
    }
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, _scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        for dy in -tiles_y..=tiles_y {
            for dx in -tiles_x..=tiles_x {
                ...
                // Request tile (Cx2d derefs to Cx)
                self.tile_cache.request_tile(cx, coord);

                // Calculate tile position on screen
                let tile_screen_x =
                    self.viewport_size.x / 2.0 + (dx as f64 * scaled_tile_size) - offset_x;
                let tile_screen_y =
                    self.viewport_size.y / 2.0 + (dy as f64 * scaled_tile_size) - offset_y;
                ...
                } else if let Some((parent_coord, uv_offset, uv_scale)) =
                    self.find_parent_tile_coord(&coord)
                {
...
impl GeoMapView {
    ...
    fn screen_to_geo(&self, screen_pos: DVec2) -> (f64, f64) {
        ...
        let lat_rad = (std::f64::consts::PI * (1.0 - 2.0 * world_y / world_size))
            .sinh()
            .atan();
    ...
    fn geo_to_screen(&self, lng: f64, lat: f64) -> DVec2 {
        ...
        let center_world_y =
            (1.0 - center_lat_rad.tan().asinh() / std::f64::consts::PI) / 2.0 * world_size;

        // Convert target to world coords
        let target_world_x = (lng + 180.0) / 360.0 * world_size;
        let target_lat_rad = lat.to_radians();
        let target_world_y =
            (1.0 - target_lat_rad.tan().asinh() / std::f64::consts::PI) / 2.0 * world_size;
    ...
    fn apply_momentum(&mut self, cx: &mut Cx, uid: WidgetUid) {
    ...
    fn emit_region_changed(&self, cx: &mut Cx, uid: WidgetUid) {
        cx.widget_action(
            uid,
...
impl GeoMapViewRef {
    ...
    pub fn tapped(&self, actions: &Actions) -> Option<(f64, f64)> {
        if let GeoMapViewAction::Tapped { lng, lat } =
    ...
    pub fn marker_tapped(&self, actions: &Actions) -> Option<LiveId> {
        if let GeoMapViewAction::MarkerTapped { id } =
            actions.find_widget_action(self.widget_uid()).cast()
        {
            Some(id)
        } else {
    ...
    pub fn region_changed(&self, actions: &Actions) -> Option<(f64, f64, f64)> {
        if let GeoMapViewAction::RegionChanged {
            center_lng,
            center_lat,
            zoom,
        } = actions.find_widget_action(self.widget_uid()).cast()
