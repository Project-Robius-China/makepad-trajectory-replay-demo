...
> 默认体验：cycling 轨迹回放。
> 长期方向：通用 trajectory replay engine，可承载 cycling / running / hiking / walking / transit 等多类轨迹数据。
...
第一版渲染层只实现 cycling 默认体验；数据层从一开始按多类型轨迹设计。也就是说：engine 能读取和归一化多类轨迹，renderer 当前只消费 cycling profile。这个边界能避免把 demo 做成封闭的一次性样品，同时控制首版实现复杂度。
...
首版选择 cycling，不是因为 engine 只能做骑行，而是因为现有 hero 数据具备丰富字段：lat / lon / ele / time / hr / cad，且存在真实高强度片段，适合验证 contract guard。后续 running / hiking / walking / transit 可复用同一数据模型，但不进入首版渲染范围。
...
它负责加载 trajectory manifest，选择默认数据集，解析 GPX 或规范化后的轨迹数据，输出统一的 `TrajectoryPoint` 和 `PlaybackState`。字段按能力分层：基础字段必需，运动字段可选，安全字段可选但若声明可用则必须可验证。
...
实现代码不在本轮编写，但 PRD 与 spec 要假定后续数据仓库会提供：
...
首版 App 可以从 GitHub Raw 拉取 manifest 或默认 GPX；失败时退回 bundled fallback。任何网络请求都必须限制在该数据 repo 的 raw URL 范围内。
...
cycling profile 额外消费 `heart_rate_bpm` 和 `cadence_rpm`；其他 profile 暂时只进入 parser / manifest / normalization 测试，不进入首版 UI 切换。
...
启动后显示“同步中...”，后台线程从 trajectory-replay-data 拉取默认 cycling 数据。成功显示“已同步”，失败显示“本地缓存”。即使网络很快，也需要保留一个可见的状态切换过程，避免用户无法确认数据来源。
...
联网只用于加载 Project-Robius-China 数据 repo 中的默认数据，不用于登录、地图、LLM、analytics 或 telemetry。

流程：

1. app 启动，`network_state = Idle`。
2. UI 显示“同步中...”，状态进入 `Fetching`。
3. 后台线程发起 HTTP GET，3 秒总超时。
...
`Fallback` 是终态，不重试。整个 90 秒默认回放期间最多发起一次数据请求。fetch 到的数据不写入磁盘 cache；每次启动都重新尝试 fetch，以便 review 时能验证联网状态机。
